...
No application logic, no transcription calls.
"""
...
class FMDemodulator:
    ...
    3. Quadrature FM demod: (I*dQ - Q*dI) / (I^2 + Q^2)
    ...
    def process(self, iq: np.ndarray) -> np.ndarray:
        ...
        # 4. Overlap-save resample to audio rate
        ...
        # 7. Apply output gain and clip
        audio = np.clip(audio * self._output_gain, -1.0, 1.0).astype(np.float32)
        
        return audio
    
    def reset(self):
        """Reset all state for new transmission."""
